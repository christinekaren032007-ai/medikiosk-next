...
export default function DemoControlPanel() {
  ...
  const loadScenario = useRaphaStore((s) => s.loadScenario);
  const resetDemo = useRaphaStore((s) => s.resetDemo);

  async function runScenario(key: "fever" | "joint_pain" | "digestive" | "headache") {
      ...
      {open && (
        <div className="bg-white border border-stone-200 rounded-2xl shadow-xl p-4 w-64">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-stone-500">DEMO CONTROLS — SIMULATED DATA</span>
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            ...
            <Button variant="amber" className="text-xs px-2 py-2" onClick={() => runScenario("joint_pain")}>Joint Pain</Button>
            <Button variant="amber" className="text-xs px-2 py-2" onClick={() => runScenario("digestive")}>Digestive</Button>
            <Button variant="amber" className="text-xs px-2 py-2" onClick={() => runScenario("headache")}>Headache</Button>
          </div>
